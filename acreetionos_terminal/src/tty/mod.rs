...
pub fn setup_env() {
    // Default to 'acreetionos-terminal' terminfo if it is available, otherwise
    // default to 'xterm-256color'. May be overridden by user's config
    // below.
    let terminfo = if terminfo_exists("acreetionos-terminal") { "acreetionos-terminal" } else { "xterm-256color" };
