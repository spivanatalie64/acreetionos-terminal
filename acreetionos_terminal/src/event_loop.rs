...
pub enum Msg {
    ...
    /// Indicates that the `EventLoop` should shut down, as AcreetionOS Terminal is shutting down.
    Shutdown,
    ...
    pub fn spawn(mut self) -> JoinHandle<(Self, State)> {
        thread::spawn_named("PTY reader", move || {
            ...
            let mut pipe = if self.ref_test {
                Some(File::create("./acreetionos-terminal.recording").expect("create acreetionos-terminal recording"))
            } else {
                None
            };
