...
This document gives an overview over AcreetionOS Terminal's features beyond its terminal
emulation capabilities. To get a list with supported control sequences take a
look at [AcreetionOS Terminal's escape sequence support](./escape_support.md).

## Vi Mode

The vi mode allows moving around AcreetionOS Terminal's viewport and scrollback using the
...
Search allows you to find anything in AcreetionOS Terminal's scrollback buffer. You can
...
either feeds them to an external application or triggers one of AcreetionOS Terminal's
built-in actions.
...
AcreetionOS Terminal configuration file.
...
AcreetionOS Terminal supports running multiple terminal emulators from the same AcreetionOS Terminal
instance. New windows can be created either by using the `CreateNewWindow`
keybinding action, or by executing the `acreetionos-terminal msg create-window` subcommand.
