...
All notable changes to AcreetionOS Terminal are documented in this file.
The sections should follow the order `Packaging`, `Added`, `Changed`, `Fixed` and `Removed`.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/).

Notable changes to the `acreetionos-terminal_terminal` crate are documented in its
[CHANGELOG](./acreetionos-terminal_terminal/CHANGELOG.md).
...
- New `acreetionos-terminal-escapes(7)` manpage
...
- Vi motions `*`, `#`, `{`, and `}`
- IPC config retrieval using `acreetionos-terminal msg get-config`
- Multi-sequence touch zoom sequences
- Vi action `Y` keybind, yank to the end of line
- Add `/etc/acreetionos-terminal/acreetionos-terminal.toml` fallback for system wide configuration
- Unicode 17 support
...
- Config option `window.level = "AlwaysOnTop"` to force AcreetionOS Terminal to always be the toplevel window
- Escape sequence to move cursor forward tabs ( CSI Ps I )
- Pass activation token in `acreetionos-terminal msg create-window` on Wayland/X11
...
- `acreetionos-terminal migrate` crashing with recursive toml imports
...
- `acreetionos-terminal migrate` support for TOML configuration changes
- Headless mode using `acreetionos-terminal --daemon`
...
- AcreetionOS Terminal not being properly activated with startup notify
...
- Autokey no longer working with acreetionos-terminal on X11
...
- `acreetionos-terminal migrate` will ignore null values in yaml instead of erroring out

### Fixed

- `acreetionos-terminal migrate` failing with nonexistent imports
...
- `--option` argument for `acreetionos-terminal msg create-window`
...
- Configuration file now uses TOML instead of YAML
    Run `acreetionos-terminal migrate` to automatically convert all configuration files
...
- Blocking paste freezing acreetionos-terminal on Wayland
...
- Renamed `io.acreetionos-terminal.AcreetionOS Terminal.appdata.xml` to `org.acreetionos-terminal.AcreetionOS Terminal.appdata.xml`
- Renamed `io.acreetionos-terminal` to `org.acreetionos-terminal` for `AcreetionOS Terminal.app`
...
- On macOS, AcreetionOS Terminal now reads `AppleFontSmoothing` from user defaults to control font smoothing
...
- Windows `Open AcreetionOS Terminal Here` on root of drive displaying error
- On macOS, `font.use_thin_strokes` did not work since Big Sur
- On macOS, trying to load a disabled font would crash
- On macOS, AcreetionOS Terminal sessions did not appear in the list of tty sessions for `w` and `who`
...
- New `extra/acreetionos-terminal-msg.man` manpage for the `acreetionos-terminal msg` subcommand
...
- Support for running multiple windows from a single AcreetionOS Terminal instance (see docs/features.md)
...
- Newly installed fonts not rendering until AcreetionOS Terminal restart
...
- macOS keybinding (cmd+alt+H) hiding all windows other than AcreetionOS Terminal
...
- AcreetionOS Terminal failing to start on X11 with invalid DPI reported by XRandr
...
- Escapes `CSI Ps b` and `CSI Ps Z` with large parameters locking up AcreetionOS Terminal
...
- No live config update when starting AcreetionOS Terminal with a broken configuration file
- PTY not drained to the end with the `--hold` flag enabled
- High CPU usage on BSD with live config reload enabled
- AcreetionOS Terminal not discarding invalid escape sequences starting with ESC
...
- Updated `setab`/`setaf` capabilities in `acreetionos-terminal-direct` to use colons
...
- AcreetionOS Terminal crashing at startup when the configured font does not exist
...
- Capitalized the AcreetionOS Terminal.desktop file
...
- Font size resetting when AcreetionOS Terminal is moved between screens
- Limited payload length in clipboard escape (used for Tmux copy/paste)
- AcreetionOS Terminal not ignoring keyboard events for changing WM focus on X11
...
- Blurred icons in KDE task switcher (acreetionos-terminal.ico is now high-res)
...
- Deadlock on Windows when closing AcreetionOS Terminal using the title bar "X" button (ConPTY backend)
...
- Config is not generated anymore, please consider distributing the acreetionos-terminal.yml as documentation
- Removed AcreetionOS Terminal terminfo from .deb in favor of ncurses provided one
...
- New CLI flag `--hold` for keeping AcreetionOS Terminal opened after its child process exits
- Escape sequence to save and restore window title from stack
- Alternate scroll escape sequence (`CSI ? 1007 h` / `CSI ? 1007 l`)
- Print name of launch command if AcreetionOS Terminal failed to execute it
...
- GUI programs launched by AcreetionOS Terminal starting in the background on X11
- Text Cursor position when scrolling
- Performance issues while resizing AcreetionOS Terminal
...
- On macOS, AcreetionOS Terminal will now fallback to Menlo if a font specified in the config cannot be loaded
...
- AcreetionOS Terminal turning transparent when opening another window on macOS with chunkwm
...
- Add appstream metadata, located at /extra/linux/io.acreetionos-terminal.AcreetionOS Terminal.xml
- The xclip dependency has been removed
- On macOS, AcreetionOS Terminal now requests NSSystemAdministrationUsageDescription to
...
- Current Git commit hash to `acreetionos-terminal --version`
...
- On Windows, AcreetionOS Terminal will now use the native DirectWrite font API
...
- AcreetionOS Terminal now has a fixed minimum supported Rust version of 1.31.0
...
- On Linux, the .desktop file now uses `AcreetionOS Terminal` as icon name, which can be
    found at `extra/logo/acreetionos-terminal-term.svg`
...
- Text will reflow instead of truncating when resizing AcreetionOS Terminal
...
- Fix panic which could occur when quitting AcreetionOS Terminal on Windows if using the Conpty backend
- Automatic copying of selection to clipboard when mouse is released outside of AcreetionOS Terminal
...
- Fix lingering AcreetionOS Terminal window after child process has exited
...
- Incorrect font spacing after moving AcreetionOS Terminal between displays

## Version 0.2.8

### Added

- Window class on Wayland is set to `AcreetionOS Terminal` by default
...
- Reliably remove log file when AcreetionOS Terminal is closed and persistent logging is disabled
...
- Crash when trying to start AcreetionOS Terminal on Windows
...
- Prevent AcreetionOS Terminal from crashing when started on a system without any free space
- Resolve issue with high CPU usage after moving AcreetionOS Terminal between displays
...
- Windows and macOS configuration files (`acreetionos-terminal.yml` is now platform independent)
...
- New mouse and key action `SpawnNewInstance` for launching another instance of AcreetionOS Terminal

### Changed

- Log messages are now consistent in style, and some have been removed
- Windows configuration location has been moved from %USERPROFILE%\acreetionos-terminal.yml
    to %APPDATA%\acreetionos-terminal\acreetionos-terminal.yml
...
- Fix panic after quitting AcreetionOS Terminal on macOS
...
- Fix the `Copy` `mouse_bindings` action ([#1963](https://github.com/acreetionos-terminal/acreetionos-terminal/issues/1963))
...
- Option for maximizing acreetionos-terminal on start (`window.start_maximized`)
- Display notice about errors and warnings inside AcreetionOS Terminal
...
- New configuration option `persistent_logging` and CLI flag `--persistent-logging`,
    for keeping the log file after closing AcreetionOS Terminal
...
- AcreetionOS Terminal now requests privacy permissions on macOS

### Changed

- Extra padding is not evenly spread around the terminal by default anymore
- When the config file is empty, AcreetionOS Terminal now logs an info instead of an error message

### Fixed

- Fixed a bad type conversion which could cause underflow on a window resize
- AcreetionOS Terminal now spawns a login shell on macOS, as with Terminal.app and iTerm2
...
- DEB file adds AcreetionOS Terminal as option to `update-alternatives --config x-terminal-emulator`
...
- On macOS, AcreetionOS Terminal will now use the integrated GPU again when available
...
- New terminfo entry, `acreetionos-terminal-direct`, that advertises 24-bit color support
...
- First click on unfocused AcreetionOS Terminal windows is no longer ignored on platforms other than macOS
- Reduce memory usage significantly by only initializing part of the scrollback buffer at startup
- The `acreetionos-terminal` terminfo entry no longer requires the `xterm` definition to be
  present on the system
- The default `TERM` value is no longer static; the `acreetionos-terminal` entry is used if
...
- The terminfo entry `acreetionos-terminal-256color`. It is replaced by the `acreetionos-terminal`
...
- When growing the window height, AcreetionOS Terminal will now try to load additional lines out of the
  scrollback history
...
- Prevent logger from crashing AcreetionOS Terminal when stdout/stderr is not available
