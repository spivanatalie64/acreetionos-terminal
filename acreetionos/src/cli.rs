...
use acreetionos_config::SerdeReplace;
use clap::{ArgAction, Args, Parser, Subcommand, ValueHint};
use log::{LevelFilter, error};
use serde::{Deserialize, Serialize};
use toml::Value;

use acreetionos_terminal::tty::Options as PtyOptions;
...
/// CLI options for the main AcreetionOS Terminal executable.
...
pub struct Options {
    ...
    /// X11 window ID to embed AcreetionOS Terminal within (decimal or hexadecimal with "0x" prefix).
    #[clap(long)]
    pub embed: Option<String>,

    /// Specify alternative configuration file [default:
    /// $XDG_CONFIG_HOME/acreetionos-terminal/acreetionos-terminal.toml].
    #[cfg(not(any(target_os = "macos", windows)))]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,

    /// Specify alternative configuration file [default: %APPDATA%\acreetionos-terminal\acreetionos-terminal.toml].
    #[cfg(windows)]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,

    /// Specify alternative configuration file [default: $HOME/.config/acreetionos-terminal/acreetionos-terminal.toml].
    #[cfg(target_os = "macos")]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,
...
pub struct WindowIdentity {
    /// Defines the window title [default: AcreetionOS Terminal].
    #[clap(short = 'T', short_alias('t'), long)]
    pub title: Option<String>,

    /// Defines window class/app_id on X11/Wayland [default: AcreetionOS Terminal].
...
/// Send a message to the AcreetionOS Terminal socket.
#[cfg(unix)]
#[derive(Args, Debug)]
...
pub enum SocketMessage {
    /// Create a new window in the same AcreetionOS Terminal process.
    CreateWindow(WindowOptions),

    /// Update the AcreetionOS Terminal configuration.
    Config(IpcConfig),

    /// Read runtime AcreetionOS Terminal configuration.
...
pub struct IpcConfig {
    ...
    #[clap(short, long, allow_hyphen_values = true, env = "ACREETIONOS_TERMINAL_WINDOW_ID")]
...
mod tests {
    ...
    fn completions() {
        ...
        for (shell, file) in &[
            (Shell::Bash, "acreetionos-terminal.bash"),
            (Shell::Fish, "acreetionos-terminal.fish"),
            (Shell::Zsh, "_acreetionos-terminal"),
        ] {
            let mut generated = Vec::new();
            clap_complete::generate(*shell, &mut clap, "acreetionos-terminal", &mut generated);
        ...
        // clap_complete::generate(Shell::Bash, &mut clap, "acreetionos-terminal", &mut file);
        // let mut file = File::create("../extra/completions/alacritty.fish").unwrap();
        // clap_complete::generate(Shell::Fish, &mut clap, "acreetionos-terminal", &mut file);
        // let mut file = File::create("../extra/completions/_alacritty").unwrap();
        // clap_complete::generate(Shell::Zsh, &mut clap, "acreetionos-terminal", &mut file);
    }
}
