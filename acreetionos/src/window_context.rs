...
use acreetionos_terminal::event_loop::{EventLoop as PtyEventLoop, Msg, Notifier};
use acreetionos_terminal::grid::{Dimensions, Scroll};
use acreetionos_terminal::index::Direction;
use acreetionos_terminal::sync::FairMutex;
use acreetionos_terminal::term::test::TermSize;
use acreetionos_terminal::term::{Term, TermMode};
...
/// Event context for one individual AcreetionOS Terminal window.
...
impl WindowContext {
    ...
    fn new(
        ...
        // Create context for the AcreetionOS Terminal window.
        Ok(WindowContext {
            preserve_title,
            terminal,
            display,
    ...
    pub fn draw(&mut self, scheduler: &mut Scheduler) {
        ...
        if !self.display.visual_bell.completed() {
            // We can get an OS redraw which bypasses acreetionos-terminal's frame throttling, thus
