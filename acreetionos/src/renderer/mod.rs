...
use acreetionos_terminal::index::Point;
...
impl Renderer {
    ...
    ///
    /// The `load_fn` parameter is used to load OpenGL function pointers. For glutin contexts,
    /// use `|sym| gl_display.get_proc_address(sym)`. For GTK GLArea contexts, use the GTK
    /// proc address function.
    #[allow(dead_code)]
    pub fn new(
        load_fn: &dyn Fn(&CStr) -> *const std::ffi::c_void,
    ...
    /// Create a renderer from a glutin context.
    pub fn from_glutin_context(
        context: &PossiblyCurrentContext,
        renderer_preference: Option<RendererPreference>,
    ) -> Result<Self, Error> {
        let gl_display = context.display();
        Self::new(
            &|symbol| gl_display.get_proc_address(symbol).cast(),
            renderer_preference,
            matches!(context.context_api(), ContextApi::Gles(_)),
        )
    }
