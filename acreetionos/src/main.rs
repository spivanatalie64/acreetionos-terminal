//! AcreetionOS Terminal - The GPU Enhanced Terminal.
...
#[cfg(not(any(feature = "gnome", feature = "x11", feature = "wayland", target_os = "macos", windows)))]
compile_error!(r#"at least one of the "gnome"/"x11"/"wayland" features must be enabled"#);
...
#[cfg(all(not(feature = "gnome"), not(any(target_os = "macos", windows))))]
...
#[cfg(all(unix, not(feature = "gnome")))]
use crate::cli::MessageOptions;
#[cfg(all(not(any(target_os = "macos", windows)), not(feature = "gnome")))]
...
/// Temporary files stored for AcreetionOS Terminal.
///
...
/// Run main AcreetionOS Terminal entrypoint.
///
/// Creates a window, the terminal state, PTY, I/O event loop, input processor,
/// config change monitor, and runs the main display loop.
#[cfg(not(feature = "gnome"))]
fn run_acreetionos(mut options: Options) -> Result<(), Box<dyn Error>> {
    ...
    info!("Welcome to AcreetionOS Terminal");
