//! Logging for AcreetionOS Terminal.
//!
...
pub const LOG_TARGET_IPC_CONFIG: &str = "acreetionos_log_window_config";

/// Name for the environment variable containing the log file's path.
const ACREETIONOS_LOG_ENV: &str = "ACREETIONOS_TERMINAL_LOG";

/// Logging target for config error messages.
pub const LOG_TARGET_CONFIG: &str = "acreetionos_config_derive";

/// Logging target for winit events.
pub const LOG_TARGET_WINIT: &str = "acreetionos_winit_event";

/// Name for the environment variable containing extra logging targets.
///
/// The targets are semicolon separated.
const ACREETIONOS_EXTRA_LOG_TARGETS_ENV: &str = "ACREETIONOS_EXTRA_LOG_TARGETS";
...
fn extra_log_targets() -> &'static [String] {
    ...
    EXTRA_LOG_TARGETS.get_or_init(|| {
        env::var(ACREETIONOS_EXTRA_LOG_TARGETS_ENV)
            .map_or(Vec::new(), |targets| targets.split(';').map(ToString::to_string).collect())
    })
}

/// List of targets which will be logged by AcreetionOS Terminal.
const ALLOWED_TARGETS: &[&str] = &[
    LOG_TARGET_IPC_CONFIG,
    LOG_TARGET_CONFIG,
    LOG_TARGET_WINIT,
    "acreetionos_config_derive",
    "acreetionos_terminal",
...
impl Logger {
    ...
    fn message_bar_log(&self, record: &log::Record<'_>, logfile_path: &str) {
        ...
        #[cfg(not(windows))]
        let env_var = format!("${ACREETIONOS_LOG_ENV}");
        #[cfg(windows)]
        let env_var = format!("%{}%", ACREETIONOS_LOG_ENV);
...
impl OnDemandLogFile {
    fn new() -> Self {
        ...
        path.push(format!("AcreetionOS-Terminal-{}.log", process::id()));

        // Set log path as an environment variable.
        unsafe { env::set_var(ACREETIONOS_LOG_ENV, path.as_os_str()) };
