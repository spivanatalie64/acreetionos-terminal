//! AcreetionOS Terminal socket IPC.
...
const ACREETIONOS_TERMINAL_SOCKET_ENV: &str = "ACREETIONOS_TERMINAL_SOCKET";
...
impl IpcListener {
    pub fn new(
        ...
        // Register socket path as environment variable for `acreetionos-terminal msg`.
        unsafe { env::set_var(ACREETIONOS_TERMINAL_SOCKET_ENV, path.as_os_str()) };
        if options.daemon {
            println!("ACREETIONOS_TERMINAL_SOCKET={}; export ACREETIONOS_TERMINAL_SOCKET", path.display());
        }
...
pub fn socket_dir() -> PathBuf {
    xdg::BaseDirectories::with_prefix("acreetionos-terminal")
...
fn find_socket(socket_path: Option<PathBuf>) -> IoResult<UnixStream> {
    ...
    if let Ok(path) = env::var(ACREETIONOS_TERMINAL_SOCKET_ENV) {
...
#[cfg(target_os = "macos")]
pub fn socket_prefix() -> String {
    String::from("AcreetionOS Terminal")
}
