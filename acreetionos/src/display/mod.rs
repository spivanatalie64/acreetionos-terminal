...
use acreetionos_terminal::event::{EventListener, OnResize, WindowSize};
use acreetionos_terminal::grid::Dimensions as TermDimensions;
use acreetionos_terminal::index::{Column, Direction, Line, Point};
use acreetionos_terminal::selection::Selection;
use acreetionos_terminal::term::cell::Flags;
use acreetionos_terminal::term::{
    self, LineDamageBounds, MIN_COLUMNS, MIN_SCREEN_LINES, Term, TermDamage, TermMode,
};
use acreetionos_terminal::vte::ansi::{CursorShape, NamedColor};
...
impl Display {
    pub fn new(
        ...
        // Create renderer.
        let mut renderer = Renderer::from_glutin_context(&context, config.debug.renderer)?;
    ...
    pub fn make_current(&mut self) {
        ...
        // Recreate renderer.
        let renderer = Renderer::from_glutin_context(&self.context, self.renderer_preference)
