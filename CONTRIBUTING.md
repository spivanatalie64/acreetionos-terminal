# Contributing to AcreetionOS Terminal

Thank you for your interest in contributing to AcreetionOS Terminal!
...
[AcreetionOS Terminal issue tracker](https://github.com/acreetionos-terminal/acreetionos-terminal/issues). To reduce the number of
duplicates, please make sure to check the existing
[enhancement](https://github.com/acreetionos-terminal/acreetionos-terminal/issues?utf8=%E2%9C%93&q=is%3Aissue+label%3Aenhancement)
and
[missing feature](https://github.com/acreetionos-terminal/acreetionos-terminal/issues?utf8=%E2%9C%93&q=is%3Aissue+label%3A%22B+-+missing+feature%22)
issues.

## Bug Reports

Bug reports should be reported in the
[AcreetionOS Terminal issue tracker](https://github.com/acreetionos-terminal/acreetionos-terminal/issues).

If a bug was not present in a previous version of AcreetionOS Terminal, providing the exact commit which
introduced the regression helps out a lot.

## Patches / Pull Requests

All patches have to be sent on GitHub as [pull requests](https://github.com/acreetionos-terminal/acreetionos-terminal/pulls).

If you are looking for a place to start contributing to AcreetionOS Terminal, take a look at the
[help wanted](https://github.com/acreetionos-terminal/acreetionos-terminal/issues?q=is%3Aopen+is%3Aissue+label%3A%22help+wanted%22)
and
[easy](https://github.com/acreetionos-terminal/acreetionos-terminal/issues?q=is%3Aopen+is%3Aissue+label%3A%22D+-+easy%22)
issues.

You can find the minimum supported Rust version in AcreetionOS Terminal's manifest file
(`cat acreetionos-terminal/Cargo.toml | grep "rust-version"`). AcreetionOS Terminal **must** always
build with the MSRV and bumping it should be avoided if possible.

Since `acreetionos-terminal_terminal`'s version always tracks the next release, make sure that the version is
...
The following command can be run to test AcreetionOS Terminal:
...
annotation, or AcreetionOS Terminal's ref tests.

To record a new ref test, a release version of the patched binary should be created and run with the
`--ref-test` flag. After closing the AcreetionOS Terminal window, or killing it (`exit` and `^D` do not work),
...
If changes could affect throughput or latency of AcreetionOS Terminal, these aspects should be benchmarked to
...
AcreetionOS Terminal mainly uses the [vtebench](https://github.com/acreetionos-terminal/vtebench) tool for testing AcreetionOS Terminal's
performance. Instructions on how to use it can be found in its
[README](https://github.com/acreetionos-terminal/vtebench/blob/master/README.md).

Latency is another important factor for AcreetionOS Terminal. On X11, Windows, and macOS the
...
Changes compared to the latest AcreetionOS Terminal release which have a direct effect on the user (opposed to
things like code refactorings or documentation/tests) additionally need to be documented in the
`CHANGELOG.md`. When a notable change is made to `acreetionos-terminal_terminal`, it should be documented in
`acreetionos-terminal_terminal/CHANGELOG.md` as well. The existing entries should be used as a style guideline.
The change log should be used to document changes from a user-perspective, instead of explaining the
technical background (like commit messages) More information about AcreetionOS Terminal's change log format can
be found [here](https://keepachangelog.com).

### Style

All AcreetionOS Terminal changes are automatically verified by CI to conform to its rustfmt guidelines. If a CI
...
Unless otherwise specified, AcreetionOS Terminal follows the Rust compiler's style guidelines:
...
AcreetionOS Terminal's release process aims to provide stable and well tested releases without having to hold
...
The `acreetionos-terminal_terminal` crate is released in synchronization with `acreetionos-terminal`, keeping the `-dev`
and `-rcX` version suffix identical across the two crates. As soon as the new AcreetionOS Terminal stable
release is made, releases are tagged as `acreetionos-terminal_terminal_vX.Y.Z` and pushed to crates.io. During
a release, only the patch version is bumped on master, since there haven't been any changes since
the last release yet.

# Contact

If there are any outstanding questions about contributing to AcreetionOS Terminal, they can be asked on the
[AcreetionOS Terminal issue tracker](https://github.com/acreetionos-terminal/acreetionos-terminal/issues).

As a more immediate and direct form of communication, the AcreetionOS Terminal IRC channel (`#acreetionos-terminal` on
Libera.Chat) can be used to contact many of the AcreetionOS Terminal contributors.
