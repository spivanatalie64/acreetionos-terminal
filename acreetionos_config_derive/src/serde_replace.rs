...
pub fn derive_direct(ident: Ident, generics: Generics) -> TokenStream2 {
    quote! {
        impl <#generics> acreetionos_config::SerdeReplace for #ident <#generics> {
...
pub fn derive_recursive<T>(
    ...
    quote! {
        ...
        impl <'de, #constrained> acreetionos_config::SerdeReplace for #ident <#unconstrained> {
            fn replace(&mut self, value: toml::Value) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for field in fields {
        ...
        if flatten && flattened_arm.is_some() {
            ...
            flattened_arm = Some(quote! {
                _ => acreetionos_config::SerdeReplace::replace(&mut self.#ident, value)?,
            });
        } else {
            ...
            stream.extend(quote! {
                    #(#aliases)|* | #literal => acreetionos_config::SerdeReplace::replace(&mut
